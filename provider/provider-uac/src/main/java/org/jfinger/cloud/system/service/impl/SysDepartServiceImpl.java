...
import org.jfinger.cloud.system.mapper.SysDepartMapper;
import org.jfinger.cloud.system.mapper.SysUserDepartMapper;
import org.jfinger.cloud.system.service.ISysDepartService;
import org.jfinger.cloud.system.utils.CodeFillUtils;
import org.jfinger.cloud.system.utils.FillRuleUtils;
...
public class SysDepartServiceImpl extends ServiceImpl<SysDepartMapper, SysDepart> implements ISysDepartService {
    ...
    public List<SysDepartTreeModel> queryMyDeptTreeList(String departIds) {
        ...
        String[] codeArr = getMyDeptParentOrgCode(departIds);
        for (int i = 0; i < codeArr.length; i++) {
            query.or().likeRight(SysDepart::getOrgCode, codeArr[i]);
        }
        query.eq(SysDepart::getStatus, CommonConstant.STATUS_DELETE);
        query.orderByAsc(SysDepart::getSort);
        //将父节点ParentId设为null
        ...
        List<SysDepartTreeModel> listResult = SysDepart.wrapTreeDataToTreeList(listDepts);
        return listResult;
    }
    ...
    public List<SysDepartTreeModel> queryTreeList() {
        ...
        // 调用wrapTreeDataToTreeList方法生成树状数据
        List<SysDepartTreeModel> listResult = SysDepart.wrapTreeDataToTreeList(list);
        return listResult;
    }
    ...
    public List<DepartIdModel> queryDepartIdTreeList() {
        ...
        List<SysDepart> list = this.list(query);
        // 调用wrapTreeDataToTreeList方法生成树状数据
        List<DepartIdModel> listResult = SysDepart.wrapTreeDataToDepartIdTreeList(list);
        return listResult;
    }
    ...
    public void saveDepartData(SysDepart sysDepart, String userName) {
        if (sysDepart != null && userName != null) {
            ...
            String[] codeArray = (String[]) FillRuleUtils.executeRule(DEPART, formData);
            //update-end--Author:baihailong  Date:20191209 for：部门编码规则生成器做成公用配置
            sysDepart.setOrgCode(codeArray[0]);
            String orgType = codeArray[1];
            sysDepart.setType(Integer.valueOf(orgType));
            sysDepart.setCreateTime(new Date());
            sysDepart.setStatus(CommonConstant.STATUS_DELETE);
            this.save(sysDepart);
        }
    ...
    private String[] generateOrgCode(String parentId) {
        ...
        if (StringUtil.isNullOrEmpty(parentId)) {
            ...
            if (departList == null || departList.size() == 0) {
                strArray[0] = CodeFillUtils.getNextYouBianCode(null);
                strArray[1] = "1";
                return strArray;
            } else {
                SysDepart depart = departList.get(0);
                oldOrgCode = depart.getOrgCode();
                orgType = depart.getType();
                newOrgCode = CodeFillUtils.getNextYouBianCode(oldOrgCode);
            }
            ...
            // 根据父级部门类型算出当前部门的类型
            orgType = depart.getType() + 1;
            // 处理同级部门为null的情况
            if (parentList == null || parentList.size() == 0) {
                // 直接生成当前的部门编码并返回
                newOrgCode = CodeFillUtils.getSubYouBianCode(parentCode, null);
            } else { //处理有同级部门的情况
                // 获取同级部门的编码,利用工具类
                String subCode = parentList.get(0).getOrgCode();
                // 返回生成的当前部门编码
                newOrgCode = CodeFillUtils.getSubYouBianCode(parentCode, subCode);
            }
        }
        // 返回最终封装了部门编码和部门类型的数组
    ...
    public void deleteBatchWithChildren(List<String> ids) {
        ...
        //根据部门id删除用户与部门关系
        userDepartMapper.delete(new LambdaQueryWrapper<SysUserDepart>().in(SysUserDepart::getDeptId, idList));
    }

    @Override
    public List<String> getSubDepIdsByDepId(String departId) {
        return this.baseMapper.getSubDepIdsByDeptId(departId);
    }
    ...
    public List<String> getMySubDepIdsByDepId(String departIds) {
        ...
        return baseMapper.getSubDeptIdsByOrgCodes(codeArr);
    }
    ...
    private void checkChildrenExists(String id, List<String> idList) {
        ...
        if (departList != null && departList.size() > 0) {
            for (SysDepart depart : departList) {
                idList.add(String.valueOf(depart.getId()));
                this.checkChildrenExists(String.valueOf(depart.getId()), idList);
            }
        }
    }
