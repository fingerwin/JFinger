...
import org.jfinger.cloud.entity.model.SysPermissionTree;
...
import org.jfinger.cloud.system.service.ISysRolePermissionService;
import org.jfinger.cloud.system.utils.PermissionDataUtils;
...
public class SysPermissionController {
    ...
    public Result<List<SysPermissionTree>> getSystemMenuList() {
        LambdaQueryWrapper<SysPermission> query = new LambdaQueryWrapper<SysPermission>();
        query.eq(SysPermission::getMenuType, MenuType.PARENT.getCode());
        query.eq(SysPermission::getStatus, CommonConstant.STATUS_NORMAL);
        query.orderByAsc(SysPermission::getSort);
        List<SysPermission> list = sysPermissionService.list(query);
        ...
        return Result.success(sysPermissionTreeList);
    }
    ...
    public Result<?> getUserPermissionByToken() {
        ...
        if (!PermissionDataUtils.hasIndexPage(metaList)) {
        ...
        LambdaQueryWrapper<SysPermission> query = new LambdaQueryWrapper<SysPermission>();
        query.eq(SysPermission::getStatus, CommonConstant.STATUS_NORMAL);
        query.eq(SysPermission::getMenuType, MenuType.BUTTON);
    ...
    public Result<SysPermission> add(@RequestBody SysPermission permission) {
        permission = PermissionDataUtils.intelligentProcessData(permission);
    ...
    public Result<SysPermission> delete(@RequestParam(name = "id") Integer id) {
    ...
    public Result<SysPermission> deleteBatch(@RequestParam(name = "ids") String ids) {
        ...
        for (String id : arr) {
            if (!StringUtils.isEmpty(id)) {
                sysPermissionService.deletePermission(Integer.valueOf(id));
            }
        }
        return Result.success("删除成功!");
    }
    ...
    public Result<List<TreeModel>> queryAsync(@RequestParam(name = "pid", required = false) Integer parentId) {
    ...
    private void getAuthJsonArray(JSONArray jsonArray, List<SysPermission> metaList) {
        for (SysPermission permission : metaList) {
            ...
            JSONObject json = null;
            if (permission.getMenuType().equals(MenuType.BUTTON.getCode()) && CommonConstant.STATUS_NORMAL.equals(permission.getStatus())) {
                json = new JSONObject();
                json.put("action", permission.getPerms());
//                json.put("type", permission.getPermsType());
    ...
    private void getPermissionJsonArray(JSONArray jsonArray, List<SysPermission> metaList, JSONObject parentJson) {
        for (SysPermission permission : metaList) {
            ...
            if (parentJson == null && tempPid == null) {
                ...
                if (permission.getMenuType().equals(MenuType.BUTTON.getCode())) {
                    ...
                    // 类型( 0：一级菜单 1：子菜单 2：按钮 )
                } else if (permission.getMenuType().equals(MenuType.CHILDREN.getCode()) || permission.getMenuType().equals(MenuType.PARENT.getCode())) {
    ...
    private JSONObject getPermissionJsonObject(SysPermission permission) {
        ...
        if (permission.getMenuType().equals(MenuType.BUTTON.getCode())) {
            ...
            //json.put("describe", permission.getName());
            return null;
        } else if (permission.getMenuType().equals(MenuType.PARENT.getCode()) || permission.getMenuType().equals(MenuType.CHILDREN.getCode())) {
            ...
            if (isWWWHttpUrl(permission.getUrl())) {
                json.put("path", EncryptUtils.md5(permission.getUrl()));
            } else {
